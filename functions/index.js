...
exports.updateUserScore = functions.database.ref('users/{userId}/answers/{questionId}').onWrite((e) => {
    ...
    return e.data.adminRef.root.child(`questions/${questionId}/choices`).once('value', (snapshot) => {
        ...
        return e.data.ref.parent.parent.child('score').once('value', (snapshot) => {
            const score = snapshot.val() || 0;
            if (userAnswer === correctAnswer) {
                return snapshot.ref.set(score + 1);
            } else if (e.data.previous.exists()) {
                return snapshot.ref.set(score - 1);
            } else {
                return snapshot.ref.set(score);
            }
        });
    });
});
